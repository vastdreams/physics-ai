"""
PATH: ai/nodal_vectorization/code_node.py
PURPOSE: Code node representation for the nodal graph.

Mathematical model:
- Each code file/module is a node in graph G = (V, E)
- Node attributes: file_path, dependencies, embeddings, metadata
- Edges: E(v_i) = {(v_i, v_j) | v_j in dependencies(v_i)}
...
class CodeNode:
    ...
    - Node: v_i in V where V is the set of all code nodes
    ...
    def __post_init__(self) -> None:
        """Initialise node after creation."""
    ...
    def _compute_hash(self) -> str:
        ...
        return hashlib.sha256(content.encode()).hexdigest()[:_HASH_TRUNCATION_LENGTH]

    def add_dependency(self, dependency: str) -> None:
        """Add a dependency to this node (self-dependencies are ignored)."""
    ...
    def set_embedding(self, embedding: List[float]) -> None:
        ...
        if embedding:
            ...
            self.logger.log(
                f"Embedding set for {self.node_id} (dim={len(embedding)})", level="DEBUG"
            )
    ...
    def get_dependency_depth(self, all_nodes: Dict[str, "CodeNode"]) -> int:
        ...
        Mathematical: depth(v_i) = 1 + max(depth(v_j) for v_j in dependencies(v_i))
