...
from utilities.state_graph import State, StateGraph, Transition, TransitionType
...
def add_state():
    ...
    try:
        ...
        _logger.log(f"Error in add_state endpoint: {str(e)}", level="ERROR")
...
def find_paths():
    ...
    try:
        ...
        cot.end_step(step_id, output_data={'error': str(e)}, validation_passed=False)
        _logger.log(f"Error in find_paths endpoint: {str(e)}", level="ERROR")
        return jsonify({'success': False, 'error': str(e)}), 500
...
def explore_scenarios():
    ...
    try:
        ...
        cot.end_step(step_id, output_data={'error': str(e)}, validation_passed=False)
        _logger.log(f"Error in explore_scenarios endpoint: {str(e)}", level="ERROR")
        return jsonify({'success': False, 'error': str(e)}), 500
